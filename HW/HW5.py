...
st.title("My HW5 Question Answering Chatbot")
...
def relevant_club_info(query):
    """
    Takes a query string as input, embeds it using OpenAI's embedding model,
    queries ChromaDB for the most relevant chunks, and returns the matching
...
if prompt := st.chat_input("What is your question?"):
    ...
    # Call relevant_club_info to perform vector search against ChromaDB.
    # This returns the most relevant document chunks for the user's query.
    rag_context = relevant_club_info(prompt)

    # --- Step 2: Invoke the LLM with the vector search results ---
    # We inject the retrieved context into the system prompt so the LLM can
    # use it to answer the user's question accurately. The LLM does NOT call
    # relevant_club_info itself — we perform the retrieval first, then pass
    # the results to the LLM as part of the system prompt context.
...
{rag_context}"""
    ...
    # Start with system prompt (always included, now enriched with RAG results)
